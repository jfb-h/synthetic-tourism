...
read_overnight_stays <- function(CONF) {
  ...
  # column art identifies cities/periods with yearly data (Munich, Nuremberg and Frankfurt)
    select(year=Jahr, month=Monat, city=Stadt, total=GAST02__Gaesteuebernachtungen__Anzahl, pop=Population, art)
}
...
extend_to_month_city_grid <- function(data, CONF) {
  ...
  filter(!(year == CONF$maxyear & month > 6)) |>
...
interpolate_quarterly_from_yearly_data <- function(data, proportions) {
      ...
      select(year, city, quarter, total, pop)
}
...
read_and_prepare_overnight_stays_and_pop <- function(CONF) {
  ...
  data_interpolation <- data |>
    ...
    summarize(total = first(total), pop = first(pop), .by = c("city", "quarter", "year")) |>
...
combine_datasets <- function(overnight_stays, gdp) {
  overnight_stays |>
    rename(stays = total) |>
...
main <- function(CONF) {
  ons <- read_and_prepare_overnight_stays_and_pop(CONF)
  # pop <- read_and_prepare_pop(CONF)
  gdp <- read_and_prepare_gdp(CONF)
  com <- combine_datasets(ons, gdp)
